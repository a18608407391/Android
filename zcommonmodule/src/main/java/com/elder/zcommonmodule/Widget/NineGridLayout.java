...
public abstract class NineGridLayout extends ViewGroup {
    ...
    public boolean compareList(List<String> urlList, List<String> ist) {
        if (urlList.size() != ist.size()) {
            return false;
        }
        int count = 0;
        for (int i = 0; i < urlList.size(); i++) {
            if (ist.contains(urlList.get(i))) {
                count++;
            }
        }
    ...
    private void layoutParams() {
        ...
        Log.e("result", "当前高度" + (int) (singleHeight * mRows + mSpacing * (mRows - 1)));
        setLayoutParams(params);
    }
    ...
    private RatioImageView createImageView(final int i, final String url) {
        ...
        imageView.setId(new Random().nextInt(255));
