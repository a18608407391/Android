...
//            Collections.reverse(lineViews);
    ...
    private void setViews() {
        ...
        for (int i = 0; i < allUrls.size(); i++) {
            ...
            RequestOptions options = new RequestOptions().error(R.drawable.icon_default).timeout(10000).transform(crop).diskCacheStrategy(DiskCacheStrategy.ALL).skipMemoryCache(true).override(400, 400);
            Glide.with(context).asBitmap().apply(options).load(allUrls.get(i)).into(imageView);
