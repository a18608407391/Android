 ...
 * 限制行的流式布局
 */
public class RestrictFlowLayout extends ViewGroup {
    /**
     * 存储行的集合，管理行
     */
    ...
    public RestrictFlowLayout(Context context) {
        this(context, null);
    }

    public RestrictFlowLayout(Context context, AttributeSet attrs) {
        super(context, attrs);

        // 获取自定义属性
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.RestrictFlowLayout);
        horizontal_space = array.getDimension(R.styleable.RestrictFlowLayout_width_space, 0);
        vertical_space = array.getDimension(R.styleable.RestrictFlowLayout_height_space, 0);
        array.recycle();
