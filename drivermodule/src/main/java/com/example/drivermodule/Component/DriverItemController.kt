...
//                .anchor(0.5f, 0.5f).icon(BitmapDescriptorFactory.fromResource(R.drawable.navi_map_gps_locked)))
    ...
    fun setLineDatas(datas: MutableList<Location>?, color: Int) {
        ...
        if (line == null) {
            line = model.mapFr.mAmap.addPolyline(PolylineOptions()
        ...
        if (line != null && !line!!.points.isNullOrEmpty()) {
            var lastPoint = line!!.points.last()
            var curPoint = datas.last()
            if (lastPoint.latitude == curPoint.latitude) {
                if (lastPoint.longitude == curPoint.longitude) {
                    return
                }
            }
        }

        Observable.just(ArrayList<LatLng>()).map(Function<ArrayList<LatLng>, ArrayList<LatLng>> {
            var v = it
            if (datas.size > 3000) {
                var da = Douglas(model.viewModel?.status.locationLat, 10.0)
                var mes = da.compress()
                mes.forEach {
                    v.add(LatLng(it.latitude, it.longitude))
                }
            } else {
                datas.forEach {
                    v.add(LatLng(it.latitude, it.longitude))
...
//            model.mapActivity.mViewModel?.changerFragment(0)
            ...
            Observable.just("").subscribeOn(Schedulers.io()).map(Function<String, String> {
                ...
                model.mapFr.mAmap.clear()
                startMarker = model.mapFr.mapUtils?.createMaker(Location(model.viewModel.status.totalPoint[0]!!.latitude, model.viewModel.status.totalPoint[0]!!.longitude))

                model.mapFr.mAmap.addMarker(MarkerOptions().position(LatLng(model.viewModel.status.totalPoint!![model.viewModel.status.totalPoint.size - 1].latitude, model.viewModel.status.totalPoint!![model.viewModel.status.totalPoint.size - 1].longitude)).anchor(0.5f, 0.5f)
                ...
                model.viewModel?.showBottomSheet?.set(true)
                model.panelState.set(SlidingUpPanelLayout.PanelState.EXPANDED)
                ...
                model.mapFr.mAmap.setOnCameraChangeListener(model.viewModel?.mapActivity.mapUtils)
            }
