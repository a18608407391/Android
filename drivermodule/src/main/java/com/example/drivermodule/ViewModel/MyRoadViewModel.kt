...
class MyRoadViewModel : BaseViewModel(), TitleComponent.titleComponentCallBack, HttpInteface.getMyRoadBook {
    ...
    fun ItemClickCommand(date: HotData) {
        var ho = PreferenceUtils.getString(myRoadBookActivity.activity, PreferenceUtils.getString(context, USERID) + "hot")
        var s = Gson().fromJson<HotData>(ho, HotData::class.java)
        if (s != null && s!!.id != date.id) {
            var dialog = DialogUtils.createNomalDialog(myRoadBookActivity.activity!!, getString(R.string.isExChangeLine), getString(R.string.cancle), getString(R.string.confirm))
            dialog.setOnBtnClickL(OnBtnClickL {
                dialog.dismiss()
            }, OnBtnClickL {
                dialog.dismiss()
                var bundle = Bundle()
                bundle.putSerializable("hotdata", date)
                myRoadBookActivity.setFragmentResult(REQUEST_LOAD_ROADBOOK, bundle)
                myRoadBookActivity._mActivity!!.onBackPressedSupport()
//                RxBus.default?.post(date)
//                ARouter.getInstance().build(RouterUtils.MapModuleConfig.MAP_ACTIVITY).addFlags(Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT).withString(RouterUtils.MapModuleConfig.RESUME_MAP_ACTIVITY, "myroad").withSerializable(RouterUtils.MapModuleConfig.RESUME_MAP_ACTIVITY_ROAD, date).navigation(myRoadBookActivity.activity, object : NavCallback() {
